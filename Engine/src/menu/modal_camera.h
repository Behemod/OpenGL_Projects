...
void Menu::camera_tab(std::shared_ptr<Camera> camera)//тут меняем то куда и от куда смотрит камера
{
    ...
    {
        camera->set_position(glm::vec3(posV[0], posV[1], posV[2]));
    }
    ImGui::LabelText("", "Camera direction");
    if (ImGui::SliderFloat3("Direction", dirV, -10.0f, 10.0f))
    {
        camera->set_direction(glm::vec3(dirV[0], dirV[1], dirV[2]));
    }
}